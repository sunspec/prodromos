...
int main (int argc, char *argv[])
{
	...
	/* Initialize the DBUS Init Flag */
	DBUS_ConfigInitFlagDone = DBUS_COMPONENT_INIT_FLAG_NO_DONE;
    ...
    do
    {
		...
		if (p_cfg->protocol == ASYNC_ETH_PARAM_PROTOCOL_TCP) {
			if (p_cfg->tcp_mode == ASYNC_ETH_PARAM_TCP_MODE_CLIENT) {
				...
				// This is the Welcome frame from the server after a succesful connection		
				memset((char *)p_cfg->p_buf_rx, 0, p_cfg->nbr_byte_from_remote_endpoint);				
				recv(p_cfg->sock_id_data, (char *)p_cfg->p_buf_rx, 6, 0);								
		...
		do
		{
		    ...
		    //@SL
			if( DBUS_ConfigInitFlagDone == DBUS_COMPONENT_INIT_FLAG_DONE )
			{
...
void  *tx_handler (void *p_arg)
{
    ...
    do
    {
		...
		//@SL{
			...
			model_state = OpalGetAsyncModelState();
			usleep(1000);
		//@SL} while ((model_state != STATE_RESET) && (model_state != STATE_STOP) && (model_state != STATE_ERROR) && (tx_error != -1));
...
static int allocate_buffers (void *p_arg)
{
	...
	{
		p_cfg->p_buf_tx = (char *)malloc(65535);
	...
	{
		p_cfg->p_buf_rx = (char *)malloc(65535);
...
static int init_tcp (void *p_arg)
{
	...
	if (p_cfg->tcp_mode == ASYNC_ETH_PARAM_TCP_MODE_SERVER)
	{
		...
		if (rc != 0)
		{
			...
			OpalPrint("%s[%d] -> Error: failed to bind TCP socket, error number %d - %s *************** \n", PROGNAME, p_cfg->ctrl_id, errno, strerror(errno) );
			return -1;
		}
...
static int receive_packet (void *p_arg)
{
	...
	AsyncEthernetCfgStruct *p_cfg = (AsyncEthernetCfgStruct *) p_arg;
	 /* DBUS Packet */
	DBUS_stPacketConfigType DBUS_stPacket;
	DBUS_stPacketConfigType *DBUS_pstPacket;
	
	/* Keep Copy of the Variable Size, we need to swap the bytes of the original */
	DBUS_u32SizeType CopyPacketSize;
	
	//OpalTrace("%s[%d] -> Enter0\n", PROGNAME);
...
#if (DBUS_SUBSCRIPTION_ENABLE == 0)
			int tx_len;
OpalTrace("%s[%d] -> Enter1\n", PROGNAME);
			DBUS_stPacket.u32Size = (DBUS_u32SubscribeComponentCountSize + DBUS_u32SubscribeComponentNameSize +
												DBUS_u32SubscribeComponentVariableCountSize + DBUS_u32SubscribeComponentVariableNameSize + 1 /* Function Command Size */);
			CopyPacketSize = DBUS_stPacket.u32Size;
			/* Swap Bytes for proper alignment */
			DBUS_stPacket.u32Size = ntohl(DBUS_stPacket.u32Size);
			/* Register Component Variables Function ID */
			DBUS_stPacket.u8Function = DBUS_FUNCTION_REQUEST_CONTENT_COMPONENT_VARIABLE;
			
			/* Copy Subscribe Components Data */
			/* Copy Subscribe Components Count to Payload */
			memcpy( (DBUS_u8PayloadDataType*)&DBUS_stPacket.u8Data[0], 
							(DBUS_u8PayloadDataType*)&DBUS_u8SubscriptionComponentCounts, DBUS_u32SubscribeComponentCountSize );

			/* Copy Subscribe Components Names to Payload */
			memcpy( (DBUS_u8PayloadDataType*)&DBUS_stPacket.u8Data[DBUS_u32SubscribeComponentCountSize], 
							(DBUS_u8PayloadDataType*)&DBUS_au8SubscriptionComponentNames, DBUS_u32SubscribeComponentNameSize );							
OpalTrace("%s[%d] -> Enter2\n", PROGNAME);
			/* Copy Variable Count to Payload */
			{
				/* Temporary swap variable */
				DBUS_u16ComponentVariableCountType TmpSwap;
				unsigned int i;
				unsigned int SingleVariableCountSize;
				
				SingleVariableCountSize = sizeof(DBUS_u16ComponentVariableCountType);
				
				for( i=0; i<DBUS_u8SubscriptionComponentCounts; i++)
				{
					/* Swap the data */
					TmpSwap = ntohs(DBUS_au8SubscriptionVariableCount[i]);
					memcpy( (DBUS_u8PayloadDataType*)&DBUS_stPacket.u8Data[DBUS_u32SubscribeComponentCountSize+DBUS_u32SubscribeComponentNameSize+(i*SingleVariableCountSize)], 
													(DBUS_u8PayloadDataType*)&TmpSwap, SingleVariableCountSize );
				}
			}
			/* Copy Variable Names to Payload */
			memcpy( (DBUS_u8PayloadDataType*)&DBUS_stPacket.u8Data[DBUS_u32SubscribeComponentCountSize+DBUS_u32SubscribeComponentNameSize+DBUS_u32SubscribeComponentVariableCountSize], 
							(DBUS_u8PayloadDataType*)&DBUS_au8SubscriptionComponentVariableName, DBUS_u32SubscribeComponentVariableNameSize );
OpalTrace("%s[%d] -> Enter3\n", PROGNAME);
			{
				int i;
				unsigned char *item;
				item = (unsigned char*)&DBUS_stPacket;
				for (i=0; i< (CopyPacketSize+4); i++) {
					OpalPrint("\tDBUS_stPacket[%d] = 0x%02X\n", i, item[i]);
				}
			}							
			
			OpalTrace("%s[%d] -> Request Components Variables onto the DBUS\n", PROGNAME, p_cfg->ctrl_id);
			tx_len = send(p_cfg->sock_id_data, (char *)&DBUS_stPacket, (CopyPacketSize+4), 0);
			//usleep(1000 * 1000);
OpalTrace("%s[%d] -> Enter4\n", PROGNAME);
#endif

			if (FD_ISSET(p_cfg->sock_id_data, &read_fd))
			{
				DBUS_u32SizeType PacketSize;
				char			 PacketErrorFlag;
				char			 PacketFunctionID;
			  	memset((char *)p_cfg->p_buf_rx, 0, 1024);
			  	//rx_len = recv(p_cfg->sock_id_data, (char *)p_cfg->p_buf_rx, (DBUS_SubscribeComponentVariableTotalSize), 0);
				...
				while(1)
				{
					...
					/* Read header data */
					rx_len = rx_len + recv(p_cfg->sock_id_data, (char *)p_cfg->p_buf_rx+rx_len, 4-rx_len, 0);
					
					if(rx_len == 4)
					{
						/* Read the packet Size */
						PacketSize = *((DBUS_u32SizeType*)p_cfg->p_buf_rx);
						/* Convert from network to program format */
						PacketSize = ntohl(PacketSize);
//OpalPrint("\tINFO - RECEIVED PACKET SIZE: %d\n", PacketSize );						
							
						/* If Packet Size is equal to 2, it's a response from the DBus Server */
						if(PacketSize == 2)
						{
						
							rx_len = recv(p_cfg->sock_id_data, (char *)p_cfg->p_buf_rx, 2, 0);
							

							PacketFunctionID = *(p_cfg->p_buf_rx + 0);							
							PacketErrorFlag = *(p_cfg->p_buf_rx + 1);
						
							if( PacketErrorFlag != 0x01 )
							{
								OpalPrint("\tError - DBUS Server Response 0x%02X for Function: 0x%02X\n", PacketErrorFlag, PacketFunctionID );
							}
							else
							{
								OpalPrint("\tSuccess - DBUS Sever Response 0x%02X for Function: 0x%02X\n", PacketErrorFlag, PacketFunctionID );
							}
							rx_len = 0;		
						}
						/* Packet size is bigger than 2, so we have some actual data to read */
						else if(PacketSize > 2)
						{
							rx_len = 0;

							do /* Make sure we read all the data */
							{
								rx_len = rx_len + recv(p_cfg->sock_id_data, (char *)p_cfg->p_buf_rx+rx_len, PacketSize-rx_len, 0);
							} while( rx_len < PacketSize); 
							
							PacketFunctionID = *(p_cfg->p_buf_rx + 0);	
							
//OpalPrint("\tINFO - RECEIVED PACKET ID 0x%02X, rx_len: %d\n", PacketFunctionID, rx_len );							
						
							if( rx_len == (DBUS_SubscribeComponentVariableTotalSize + 1) )
							{
								OpalPrint("\tSuccess - Received Packet Size: %d for Function: 0x%02X\n", PacketSize, PacketFunctionID );
							}
							else
							{
								OpalPrint("\tError - Received Packet Size: %d for Function: 0x%02X, Rx_Len: %d, Expected Rx_Len: %d\n", PacketSize, PacketFunctionID, rx_len, (DBUS_SubscribeComponentVariableTotalSize) );
							}
							break;
				...
				if (rx_len > 0) 
				{
					...
					int i;
					for (i=0; i< rx_len; i++) {
						OpalPrint("\tp_buf_rx[%d] = 0x%02X\n", i, (unsigned char)(p_cfg->p_buf_rx[i]) );
					}
				}  
/* 				else {
					OpalTrace("%s[%d] -> Received no reply from DBUS...\n", PROGNAME, p_cfg->ctrl_id);
					return 0;
				  }	 */	
	...
	if (rx_len == (DBUS_SubscribeComponentVariableTotalSize + 1) )
	{
		...
		for (rx_data_cnt = 0; rx_data_cnt < DBUS_u32SubscribeComponentVariableTypeSize; rx_data_cnt++) {
			switch(DBUS_au8SubscriptionComponentVariableType[rx_data_cnt])
			{
				/* BOOL - Byte Size */
				case DBUS_COMPONENT_VARIABLE_BOOL_TYPE:
				    p_cfg->p_buf_to_model[rx_data_cnt] = (unsigned char)*p_buf;
					p_buf += sizeof(unsigned char);
					break;
				/* Signed Integer */
				case DBUS_COMPONENT_VARIABLE_INTERGER_TYPE:
					if(p_cfg->swap_rx)			   
					...
					break;
				/* Single Float */
				case DBUS_COMPONENT_VARIABLE_SINGLE_TYPE:
					if(p_cfg->swap_rx)
						swap32((void*)p_buf_f32);					
					p_cfg->p_buf_to_model[rx_data_cnt] = (float)*p_buf_f32;
					//p_cfg->p_buf_to_model[rx_data_cnt] = (float)rx_data_cnt;
					p_buf_f32++;
					break;
				/* Double Float */
				case DBUS_COMPONENT_VARIABLE_DOUBLE_TYPE:
					if(p_cfg->swap_rx)
...
OpalPrint("\tINFO - Sending data to model...\n" );
...
static int transmit_packet (void *p_arg)
{
	...
	if (p_cfg->protocol == ASYNC_ETH_PARAM_PROTOCOL_TCP)
	{
		...
		int rc;
    	struct  timeval tv;
	    //char register_cmd[] = "\x00\x00\x00\x08\x0A\x0F\x4D\x61\x73\x74\x65\x72";
		char register_cmd[] = "\x00\x00\x00\x0C\x0A\x01\x4f\x50\x41\x4c\x52\x54\x5f\x48\x49\x4c";
	    char register_response[] = "\x00\x00\x00\x02\x0A\x01";
	    char register_component_response[] = "\x00\x00\x00\x02\x0B\x01";
		char subscribe_component_response[] = "\x00\x00\x00\x02\x0D\x01";
		char updated_variable_response[] = "\x00\x00\x00\x02\x0F\x01";
		...
		if( DBUS_ConfigInitFlagDone != DBUS_COMPONENT_INIT_FLAG_DONE )
		{
			...
			/****************************************************************************************************************/
			/* Register the Component on the DBUS 																			*/
			/****************************************************************************************************************/
			
			OpalTrace("%s[%d] -> Registering to DBUS\n", PROGNAME, p_cfg->ctrl_id);
			
			DBUS_stPacket.u32Size = (DBUS_u32ComponentTypeDefinitionSize + DBUS_u32ComponentNameDefinitionSize + 1 /* Function Command Size */);
			CopyPacketSize = DBUS_stPacket.u32Size;
			/* Swap Bytes for proper alignment */
			DBUS_stPacket.u32Size = ntohl(DBUS_stPacket.u32Size);
			/* Register Component Function ID */
			DBUS_stPacket.u8Function = DBUS_FUNCTION_REGISTER_COMPONENT;
			
			/* Copy Registering Component Data */
			/* Copy Component Type to Payload */
			memcpy( (DBUS_u8PayloadDataType*)&DBUS_stPacket.u8Data[0], (DBUS_u8PayloadDataType*)&DBUS_u8ComponentType, DBUS_u32ComponentTypeDefinitionSize );
			/* Copy Component Name to Payload */
			memcpy( (DBUS_u8PayloadDataType*)&DBUS_stPacket.u8Data[DBUS_u32ComponentTypeDefinitionSize], 
							(DBUS_u8PayloadDataType*)DBUS_au8ComponentName, DBUS_u32ComponentNameDefinitionSize );
			...
			while(1)
			{
				rx_len = recv(p_cfg->sock_id_data, (char *)p_cfg->p_buf_rx, 16, 0);
				
				if(rx_len > 4)
				{
					//OpalPrint("\trx_len = %d, Error no %d -  %s\n", rx_len, errno, strerror(errno));
					break;
				}
			...
			OpalPrint("\trx_len = %d\n", rx_len);
			 ...
			 if (rx_len >0) {
				OpalTrace("%s[%d] -> Received reply from DBUS (%d bytes) -> \n", PROGNAME, p_cfg->ctrl_id,rx_len);
				...
				rc = memcmp(register_response, p_cfg->p_buf_rx, 6);
				if (rc == 0) {
					OpalTrace("%s[%d] -> Successfully registered as a component on DBUS server!\n", PROGNAME, p_cfg->ctrl_id);
				} else {
					OpalTrace("%s[%d] -> Failed to register component on DBUS server!\n", PROGNAME, p_cfg->ctrl_id);
					return 0;
				}
			...
			/****************************************************************************************************************/
			/* Register the Variables on the DBUS 																			*/
			/****************************************************************************************************************/			  
 
			DBUS_stPacket.u32Size = (DBUS_u32ComponentVariableCountSize + DBUS_u32ComponentVariableNameSize +
												DBUS_u32ComponentVariableTypeSize + 1 /* Function Command Size */);
			CopyPacketSize = DBUS_stPacket.u32Size;
			/* Swap Bytes for proper alignment */
			DBUS_stPacket.u32Size = ntohl(DBUS_stPacket.u32Size);
			/* Register Component Variables Function ID */
			DBUS_stPacket.u8Function = DBUS_FUNCTION_REGISTER_COMPONENT_VARIABLE;
			
			
			/* Copy Component Variable Data */
			/* Copy Variable Count to Payload */
			{
				/* Temporary swap variable */
				DBUS_u16ComponentVariableCountType TmpSwap;
				/* Swap the data */
				TmpSwap = ntohs(DBUS_u16ComponentVariableCount);
				memcpy( (DBUS_u8PayloadDataType*)&DBUS_stPacket.u8Data[0], (DBUS_u8PayloadDataType*)&TmpSwap, DBUS_u32ComponentVariableCountSize );
			}
			/* Copy Variable Names to Payload */
			memcpy( (DBUS_u8PayloadDataType*)&DBUS_stPacket.u8Data[DBUS_u32ComponentVariableCountSize], 
							(DBUS_u8PayloadDataType*)DBUS_au8ComponentVariableName, DBUS_u32ComponentVariableNameSize );
			/* Copy Variable Types to Payload */
			memcpy( (DBUS_u8PayloadDataType*)&DBUS_stPacket.u8Data[DBUS_u32ComponentVariableCountSize+DBUS_u32ComponentVariableNameSize], 
							(DBUS_u8PayloadDataType*)DBUS_au8ComponentVariableType, DBUS_u32ComponentVariableTypeSize );
			...
			OpalTrace("%s[%d] -> Registering Components Variables onto the DBUS\n", PROGNAME, p_cfg->ctrl_id);
			tx_len = send(p_cfg->sock_id_data, (char *)&DBUS_stPacket, (CopyPacketSize+4), 0);
			...
			if (rx_len > 0) {
				...
				rc = memcmp(register_component_response, p_cfg->p_buf_rx, 6);
				if (rc == 0) {
					OpalTrace("%s[%d] -> Successfully registered component variables on DBUS server!\n", PROGNAME, p_cfg->ctrl_id);
				} else {
					OpalTrace("%s[%d] -> Failed to register component variables on DBUS server!\n", PROGNAME, p_cfg->ctrl_id);
				}
			...
			/****************************************************************************************************************/
			/* Subscribe to Variables on other Components on the DBUS 														*/
			/****************************************************************************************************************/
#if (DBUS_SUBSCRIPTION_ENABLE == 1)
			unsigned int SingleVariableCountSize;
				
			SingleVariableCountSize = sizeof(DBUS_u16ComponentVariableCountType);
			
			DBUS_stPacket.u32Size = (DBUS_u32SubscribeComponentCountSize + DBUS_u32SubscribeComponentNameSize +
												(DBUS_u32SubscribeComponentVariableCountSize*SingleVariableCountSize) + DBUS_u32SubscribeComponentVariableNameSize + 1 /* Function Command Size */);
			CopyPacketSize = DBUS_stPacket.u32Size;
			/* Swap Bytes for proper alignment */
			DBUS_stPacket.u32Size = ntohl(DBUS_stPacket.u32Size);
			/* Register Component Variables Function ID */
			DBUS_stPacket.u8Function = DBUS_FUNCTION_SUBSCRIBE_COMPONENT_VARIABLE;
			...
			/* Copy Variable Count to Payload */
			{
				...
				for( i=0; i<DBUS_u32SubscribeComponentVariableCountSize; i++)
				{
					/* Swap the data */
					TmpSwap = ntohs(DBUS_au8SubscriptionVariableCount[i]);
					memcpy( (DBUS_u8PayloadDataType*)&DBUS_stPacket.u8Data[DBUS_u32SubscribeComponentCountSize+DBUS_u32SubscribeComponentNameSize+(i*SingleVariableCountSize)], 
			...
			/* Copy Variable Names to Payload */
			memcpy( (DBUS_u8PayloadDataType*)&DBUS_stPacket.u8Data[DBUS_u32SubscribeComponentCountSize+DBUS_u32SubscribeComponentNameSize+(DBUS_u32SubscribeComponentVariableCountSize*SingleVariableCountSize)], 
							(DBUS_u8PayloadDataType*)&DBUS_au8SubscriptionComponentVariableName, DBUS_u32SubscribeComponentVariableNameSize );
			...
			OpalTrace("%s[%d] -> Subscribe Components Variables onto the DBUS\n", PROGNAME, p_cfg->ctrl_id);
			tx_len = send(p_cfg->sock_id_data, (char *)&DBUS_stPacket, (CopyPacketSize+4), 0);
			...
			if (rx_len > 0) {
				...
				rc = memcmp(subscribe_component_response, p_cfg->p_buf_rx, 6);
				if (rc == 0) {
					OpalTrace("%s[%d] -> Successfully Subscribe component variables on DBUS server!\n", PROGNAME, p_cfg->ctrl_id);
				} else {
					OpalTrace("%s[%d] -> Failed to Subscribe component variables on DBUS server!\n", PROGNAME, p_cfg->ctrl_id);
				}
			...
			{
				unsigned int i;
				/* Init the total size in bytes of the update variable packet */
				/* This value is computed once during the init part, tells the total size in bytes of all the variables */
				DBUS_UpdateVariablesTotalSize = 0;
				for( i=0; i < DBUS_u16ComponentVariableCount; i++ )
				{
					switch( DBUS_au8ComponentVariableType[i] )
					{
						/* BOOL - Byte Size */
						case DBUS_COMPONENT_VARIABLE_BOOL_TYPE:
							DBUS_UpdateVariablesTotalSize = DBUS_UpdateVariablesTotalSize + 1;
							break;
						/* Signed Integer */
						case DBUS_COMPONENT_VARIABLE_INTERGER_TYPE:
							DBUS_UpdateVariablesTotalSize = DBUS_UpdateVariablesTotalSize + 4;
							break;
						/* Single Float */
						case DBUS_COMPONENT_VARIABLE_SINGLE_TYPE:
						    DBUS_UpdateVariablesTotalSize = DBUS_UpdateVariablesTotalSize + 4;
						...
						/* Double Float */
						case DBUS_COMPONENT_VARIABLE_DOUBLE_TYPE:
						    DBUS_UpdateVariablesTotalSize = DBUS_UpdateVariablesTotalSize + 8;
							break;
				...
				DBUS_SubscribeComponentVariableTotalSize = 0;
				
				for( i=0; i < DBUS_u32SubscribeComponentVariableTypeSize; i++ )
				{
					switch( DBUS_au8SubscriptionComponentVariableType[i] )
					{
						/* BOOL - Byte Size */
						case DBUS_COMPONENT_VARIABLE_BOOL_TYPE:
							DBUS_SubscribeComponentVariableTotalSize = DBUS_SubscribeComponentVariableTotalSize + 1;
							break;
						/* Signed Integer */
						case DBUS_COMPONENT_VARIABLE_INTERGER_TYPE:
							DBUS_SubscribeComponentVariableTotalSize = DBUS_SubscribeComponentVariableTotalSize + 4;
							break;
						/* Single Float */
						case DBUS_COMPONENT_VARIABLE_SINGLE_TYPE:
						    DBUS_SubscribeComponentVariableTotalSize = DBUS_SubscribeComponentVariableTotalSize + 4;
						...
						/* Double Float */
						case DBUS_COMPONENT_VARIABLE_DOUBLE_TYPE:
						    DBUS_SubscribeComponentVariableTotalSize = DBUS_SubscribeComponentVariableTotalSize + 8;
							break;
		...
		usleep(1000 * 1000);
	    OpalTrace("%s[%d] -> Registering to DBUS\n", PROGNAME, p_cfg->ctrl_id);
	    tx_len = send(p_cfg->sock_id_data, (char *)&register_cmd[0], sizeof(register_cmd)-1,0);
		 ...
		 if (rx_len >0) {
		  	...
		  	if (rc == 0) {
		  		...
		  		OpalTrace("%s[%d] -> Failed to register as a unique master component on DBUS server!\n", PROGNAME, p_cfg->ctrl_id);
		  		return 0;
		  	}
	    ...
	    memset((char *)p_cfg->p_buf_rx, 0, p_cfg->nbr_byte_from_remote_endpoint);	
	    char register_components_cmd[] = "\x00\x00\x00\x34\x0B\x05\x76\x6F\x6C\x74\x61\x67\x65\x2C\x63\x75\x72\x72\x65\x6E\x74\x2C\x69\x74\x65\x72\x61\x74\x69\x6F\x6E\x73\x2C\x74\x61\x70\x20\x63\x68\x61\x6E\x67\x65\x73\x2C\x61\x72\x6D\x65\x64\x2C\x02\x02\x01\x01\x04";
		
		
		OpalTrace("%s[%d] -> Registering components Variables onto the DBUS\n", PROGNAME, p_cfg->ctrl_id);
	    tx_len = send(p_cfg->sock_id_data, (char *)&register_components_cmd[0], sizeof(register_components_cmd)-1,0);
		...
		/****************************************************************************************************************/
		/* Request Info about System's Components on the DBUS 															*/
		/****************************************************************************************************************/
				
		   memset((char *)p_cfg->p_buf_rx, 0, p_cfg->nbr_byte_from_remote_endpoint);	
		   char request_components_cmd[] = "\x00\x00\x00\x01\x0C"; 
		   
		   OpalTrace("%s[%d] -> Requesting information on registered components...\n", PROGNAME, p_cfg->ctrl_id);
		   tx_len = send(p_cfg->sock_id_data, (char *)&request_components_cmd[0], sizeof(request_components_cmd)-1,0);
			
			//usleep(1000 * 1000);
			//rx_len = recv(p_cfg->sock_id_data, (char *)p_cfg->p_buf_rx, 1024, 0);
			...
			Size = ((int)p_cfg->p_buf_rx[0] << 24);
			Size = Size | ((int)p_cfg->p_buf_rx[1] << 16);
			Size = Size | ((int)p_cfg->p_buf_rx[2] << 8);
			Size = Size | ((int)p_cfg->p_buf_rx[3]);
			
			//unsigned int val;
			
			//val = atoi(&pSize);
			
			OpalTrace("%s[%d] -> Information message contains %d bytes... ok\n", PROGNAME, p_cfg->ctrl_id, Size );
			
			usleep(1000 * 1000);
		
			/* Init Done */
			DBUS_ConfigInitFlagDone = DBUS_COMPONENT_INIT_FLAG_DONE;
		}
		
		/****************************************************************************************************************/
		/* Update Components Variables Coming from the Model																				*/
		/****************************************************************************************************************/
	
		/* Check that we have the same number of variable coming from the model that we defined here */
		if( DBUS_u16ComponentVariableCount != (p_cfg->nbr_to_remote_endpoint - (p_cfg->offset_to_remote_endpoint+7)/8))
		{
			OpalPrint("%s[%d] -> Error: Mismatch Size between the Model send data size and the configured data size\n", PROGNAME, p_cfg->ctrl_id);	
			return 0;
		}
			
		p_buf = (char*)&DBUS_stPacket.u8Data[0];


		p_buf_32  = (unsigned int *)  	p_buf;
		p_buf_f32 = (float *)         	p_buf;
		p_buf_64  = (double *)        	p_buf;
		for (tx_data_cnt = 0; tx_data_cnt < DBUS_u16ComponentVariableCount; tx_data_cnt++) 
		{
			switch( DBUS_au8ComponentVariableType[tx_data_cnt] )
			{
				/* BOOL - Byte Size */
				case DBUS_COMPONENT_VARIABLE_BOOL_TYPE:
			...
			OpalTrace("%s[%d] -> Update Component's Variables to DBUS\n", PROGNAME, p_cfg->ctrl_id);
			
			DBUS_stPacket.u32Size = (DBUS_UpdateVariablesTotalSize + 1 /* Function Command Size */);
			CopyPacketSize = DBUS_stPacket.u32Size;
			/* Swap Bytes for proper alignment */
			DBUS_stPacket.u32Size = ntohl(DBUS_stPacket.u32Size);
			/* Register Component Function ID */
			DBUS_stPacket.u8Function = DBUS_FUNCTION_UPDATE_COMPONENT_VARIABLE;
			...
			while(1)
			{
			 ...
			 if (rx_len >0) {
				...
				rc = memcmp(updated_variable_response, p_cfg->p_buf_rx, 6);
				if (rc == 0) {
					OpalTrace("%s[%d] -> Successfully Updated Component's Variable on DBUS server!\n", PROGNAME, p_cfg->ctrl_id);
				} else {
					OpalTrace("%s[%d] -> Failed to Updated Component's Variable on DBUS server!\n", PROGNAME, p_cfg->ctrl_id);
					return 0;
				}
