...
def readConfigFrame2(cli, debug=True):
    '''
...
def getDataSample(rcvr, debug=False):
    ...
    '''
    fullHexStr = ""
    if type(rcvr) == "client" or type(rcvr) == Client:
        # if debug: print('*** Client Get Data Sample ***')
        introBytesStr = rcvr.readSample(4)
        introHexStr = bytesToHexStr(introBytesStr)
